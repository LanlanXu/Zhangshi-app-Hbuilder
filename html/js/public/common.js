(function (w) {
	...
	function plusReady () {
		...
		if (curId == '/barcode_scan.html') {
			// 状态条设置为红色
			plus.navigator.setStatusBarBackground('#000');
		}
		else if (curId != 'photos') {
			// 状态条设置为红色
		...
		plus.key.addEventListener('backbutton', function () {
			...
			// 当有子窗口时点击系统的返回按钮，由父页面执行范围操作
			if (/childwebview/.test(plus.webview.currentWebview().id)) {
				var topId = plus.webview.getTopWebview().id,
					topWebview = plus.webview.getWebviewById(topId);
				topWebview.close('auto');
			} else {
				back();
			}
	...
	// 处理点击事件
	var openw = null, waiting = null, bool = true, timer = null;
	/**
	 * 打开新窗口
	 ...
	 */
	w.clicked = function (id, str, wa, ns, ws) {
		...
		if (w.plus) {
			...
			wa && (waiting = plus.nativeUI.showWaiting('', { style: 'black', modal: false, background: 'rgba(0,0,0,0)' }));
			ws = ws || {};
			...
			ns || openw.addEventListener('loaded', function () {//页面加载完成后才显示
				...
				if (str == 'reloadIndex') {
					// setTimeout(function(){
					// 	var cur = plus.webview.getWebviewById( plus.runtime.appid );
					// 	cur.loadURL("/index.html");
					// },200);
				}
				if (bool) {
					openw.show(as);
	...
	/**
	 * 给数字补零，比如说1补两位，就变成01，补8位就是00000001
...
; (function () {
	...
	function FastClick (layer, options) {
		...
		var methods = ['onMouse', 'ontouchstart', 'onTouchStart', 'onTouchMove', 'onTouchEnd', 'onTouchCancel'];
		var context = this;
		...
		layer.addEventListener('click', this.ontouchstart, true);
		...
		// If a handler is already declared in the element's ontouchstart attribute, it will be fired before
		// FastClick's ontouchstart handler. Fix this by pulling out the user-defined handler function and
		// adding it as listener.
		if (typeof layer.ontouchstart === 'function') {

			// Android browser on at least 3.2 requires a new reference to the function in layer.ontouchstart
			// - the old one won't work if passed to addEventListener directly.
			oldontouchstart = layer.ontouchstart;
			layer.addEventListener('click', function (event) {
				oldontouchstart(event);
			}, false);
			layer.ontouchstart = null;
		}
	}
	...
	*/
	...
	FastClick.prototype.destroy = function () {
		...
		layer.removeEventListener('click', this.ontouchstart, true);
